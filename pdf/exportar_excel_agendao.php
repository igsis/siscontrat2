...
$objPHPExcel->setActiveSheetIndex(0)
    ...
    ->setCellValue("K1", "Horário do fim")
    ->setCellValue("L1", "Nº de Apresentações")
    ->setCellValue("M1", "Período")
    ->setCellValue("N1", "Ação / Expressão Artística Principal")
...
while($linha = mysqli_fetch_array($query))
{
    ...
    $objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue($h, exibirDataBr($linha['data_inicio']))
        ->setCellValue($i, ($linha['data_fim'] == "0000-00-00") ? "Não é Temporada" : exibirDataBr($linha['data_fim']))
        ->setCellValue($j, exibirHora($linha['hora_inicio']))
        ->setCellValue($k, exibirHora($linha['hora_fim']))
        ...
        ->setCellValue($r, $linha['valor_ingresso'] != '0.00' ? dinheiroParaBr($linha['valor_ingresso']) . " reais." : "Gratuito")
