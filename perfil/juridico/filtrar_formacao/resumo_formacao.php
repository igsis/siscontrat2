        ...
        <form action="?perfil=juridico&p=filtrar_formacao&sp=detalhe_formacao" method="post">
            ...
            <button type="submit" name="detalhe" class="btn btn-info pull-left">Detalhe Formação
                ...
                <div class="box-footer">
                    <form action='?perfil=juridico&p=filtrar_formacao&sp=modelo_final_formacao' method='post' target="_blank">
                        <input type='hidden' name='idFormacao' value='<?= $idFormacao ?>'>
                        <input type='hidden' name='idPedido' value='<?= $idPedido ?>'>
                        <button name='doc' class='btn btn-success center-block' style='width: 30%' type='submit'>Gerar documentos</button>
                    </form>
                </div>
                <?php } ?>
            </div>
